...
- 本地部署源默认是 `/Volumes/Data/Projects/video-api-debugger-v12-full-todo`。2026-09-20 图片生成发布采用独立分支 `codex/image-studio-release`，线上代码提交 `6a95da91eeb6ac0b6c4bc6e1f571f895cae746b5`（v0.3.0）；后续操作先重新核对线上 `.deployed-commit`，不得将开发分支整体覆盖生产。开发分支 `codex/gpt-image-studio` 还包含未发布的旧视频批量下载修改，不属于本次发布。服务器目录里的 `.git` 不作为可信部署来源；不要默认在服务器上 `git pull`。图片生成配置、后台进程与验收记录见 `tasks/todo/2026-09-20-gpt-image-studio.md`。
