...
export function TemplateLibraryClient() {
  ...
  const [newTemplateDraft, setNewTemplateDraft] = useState<TemplateConfigApiDraft | null>(null);
  ...
  const [newTemplateNotice, setNewTemplateNotice] = useState<string | null>(null);
  ...
  const handleGenerateNewTemplateDraft = useCallback(async () => {
    if (!newTemplateIntent.trim()) {
      setNewTemplateError('请先描述要创建的模板。');
      return;
    }

    setNewTemplateBusy(true);
    setNewTemplateError(null);
    setNewTemplateNotice(null);
    setNewTemplateDraft(null);
    try {
      const response = await fetch('/api/templates/config-builder/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ intent: newTemplateIntent }),
      });
      const data = await response.json() as TemplateConfigApiResponse;
      if (!response.ok) {
        setNewTemplateError(data.error || '模板草稿生成失败');
        return;
      }
      if (data.needs_clarification) {
        setNewTemplateNotice(`LLM 需要先追问：${(data.questions || []).join(' / ') || '缺少关键信息'}`);
        return;
      }
      if (!data.draft) {
        setNewTemplateError((data.validation_errors || []).join(' / ') || 'LLM 未返回可保存的模板草稿');
        return;
      }
      setNewTemplateDraft(data.draft);
      setNewTemplateNotice(data.validation_errors?.length
        ? `已收到草稿，但结构化校验有 ${data.validation_errors.length} 个提示。`
        : '已生成结构化模板草稿，确认后可保存。');
    } catch (generateError) {
      setNewTemplateError(generateError instanceof Error ? generateError.message : '模板草稿生成失败');
    } finally {
      setNewTemplateBusy(false);
    }
  }, [newTemplateIntent]);

  const handleSaveNewTemplateDraft = useCallback(async () => {
    if (!newTemplateDraft) return;
    ...
    try {
      const response = await fetch('/api/templates/config-builder/save', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ draft: newTemplateDraft, mode: 'draft' }),
      });
      const data = await response.json() as TemplateConfigApiResponse;
      if (!response.ok || !data.template) {
        setNewTemplateError(data.error || '保存新模板失败');
        return;
      }
      setNewTemplateNotice(`已保存为草稿模板：${data.template.name}`);
      setNewTemplateDraft(null);
      setNewTemplateIntent('');
      await loadTemplates(data.template.id);
    } catch (saveNewTemplateError) {
      setNewTemplateError(saveNewTemplateError instanceof Error ? saveNewTemplateError.message : '保存新模板失败');
    } finally {
      setNewTemplateSaving(false);
    }
  }, [loadTemplates, newTemplateDraft]);
  ...
  return (
    <div className="template-library-page">
      ...
      {canManageTemplates && (
        <section className="template-library-llm-create" aria-label="LLM 新建模板">
          <div>
            <span className="template-library-kicker">Template Config Agent</span>
            <h2>LLM 新建模板</h2>
            <p>描述模板用途，系统生成模块规划、提示词格式、规则、素材绑定和 Temporal 草稿。</p>
          </div>
          <div className="template-library-llm-create-form">
            <textarea
              value={newTemplateIntent}
              onChange={(event) => setNewTemplateIntent(event.currentTarget.value)}
              rows={3}
              placeholder="例如：创建一个品牌宣传用的兔子 IP 视频模板，需要角色一致、Logo 稳定露出、15 秒分段。"
            />
            <div className="template-library-llm-create-actions">
              <button type="button" onClick={handleGenerateNewTemplateDraft} disabled={newTemplateBusy}>
                {newTemplateBusy ? '生成中...' : newTemplateDraft ? '重新生成草稿' : '生成模板草稿'}
              </button>
              <button type="button" onClick={handleSaveNewTemplateDraft} disabled={!newTemplateDraft || newTemplateSaving}>
                {newTemplateSaving ? '保存中...' : '保存为草稿模板'}
              </button>
              <Link href="/admin/integrations">API 设置</Link>
            </div>
            {newTemplateError && <div className="template-library-llm-message is-error">{newTemplateError}</div>}
            {newTemplateNotice && <div className="template-library-llm-message">{newTemplateNotice}</div>}
            {newTemplateDraft && (
              <div className="template-library-llm-preview">
                <strong>{newTemplateDraft.templateDraft.name}</strong>
                <p>{newTemplateDraft.templateDraft.description || 'LLM 已生成可保存的模板结构。'}</p>
                <dl>
                  <div><dt>模块</dt><dd>{newTemplateDraft.modulePlan.map((item) => item.name).join(' / ') || '待补充'}</dd></div>
                  <div><dt>规则</dt><dd>{newTemplateDraft.rules.length} 条</dd></div>
                  <div><dt>Temporal</dt><dd>{newTemplateDraft.temporal.enabled ? `${newTemplateDraft.temporal.segment}s 分段` : '未启用'}</dd></div>
                  <div><dt>缺口</dt><dd>{newTemplateDraft.missingInputs.length ? newTemplateDraft.missingInputs.join(' / ') : '无'}</dd></div>
                </dl>
              </div>
            )}
          </div>
