...
function safeVideoPath(segments: string[]) {
  if (!segments.length) return null;
  if (segments.some((segment) => !segment || segment === '.' || segment === '..' || segment.includes('/') || segment.includes('\\'))) {
    return null;
  }

  const filePath = path.normalize(path.join(VIDEOS_ROOT, ...segments));
  const relativePath = path.relative(VIDEOS_ROOT, filePath);
...
async function videoResponse(
  ...
  const filePath = safeVideoPath(params.path || []);
  if (!filePath) {
    return NextResponse.json({ error: '视频文件路径无效' }, { status: 400 });
  }
...
export async function GET(
  ...
  return videoResponse(request, params, true);
}
...
export async function HEAD(
  ...
  return videoResponse(request, params, false);
}
