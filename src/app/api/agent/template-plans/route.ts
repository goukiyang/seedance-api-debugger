        ...
        data: {
          ...
          steps: {
            create: [
              ...
              {
                step_key: 'module_composer',
                title: '模块组合',
                input_json: JSON.stringify({ module_bindings: template.module_bindings, assets: template.assets }),
                output_json: JSON.stringify({ character: template.module_bindings.character, logo: template.module_bindings.logo, style: template.module_bindings.style }),
                sort_order: 3,
              },
              {
                step_key: 'prompt_compose',
                title: 'Prompt 生成',
                input_json: JSON.stringify({ template: template.template_key, rules: template.rules, prompts: template.prompts }),
              ...
              },
              {
                step_key: 'validator',
                title: '提交校验',
                input_json: JSON.stringify({ text: input.text, selectedPlan: recommendedPlanKey }),
                output_json: JSON.stringify({ valid: true, message: '等待用户确认方案并提交 Seedance' }),
                sort_order: 7,
              },
              {
                step_key: 'seedance_execution',
                title: 'Seedance 执行',
                input_json: JSON.stringify({ selectedPlan: recommendedPlanKey, prompt: recommendedPrompt }),
                output_json: JSON.stringify({ status: 'pending_submit', message: '尚未提交生成任务' }),
                sort_order: 8,
              },
              {
