...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    verified = verifyCreditCallback(Buffer.concat(chunks).toString('utf8'), request.headers, {
  ...
  if (verified.challenge) return NextResponse.json({ challenge: verified.challenge });
  try {
    const event = verified.event;
    const value = event?.action?.value;
    if (!value || typeof value.requestId !== 'string' || !['prepare', 'confirm', 'back', 'reject'].includes(value.action)) throw new CreditRequestError('卡片操作无效');
