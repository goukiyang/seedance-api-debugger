...
type AuditTask = {
  id: string;
  ownerId: string;
  owner: { name: string | null; username: string; email: string } | null;
  model: string;
  status: string;
  prompt: string;
  unitCredits: number;
...
export default function AdminImageStudioClient() {
  ...
  async function download(id: string) {
    ...
    if (!response.ok) { const value = await response.json().catch(() => ({})); setError(value.error || '下载失败'); return; }
    const blob = await response.blob();
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement('a'); anchor.href = url; anchor.download = `image-studio-${id.slice(0, 10)}.png`; anchor.click();
  ...
  return <div className="admin-integrations-page">
    ...
    <section className="card">
      ...
      {tasks.length > 0 && <div style={{ overflowX: 'auto' }}><table className="table"><thead><tr><th>时间</th><th>用户</th><th>模型</th><th>状态</th><th>画面描述</th><th>结果</th><th /></tr></thead><tbody>{tasks.map(task => <tr key={task.id}>
        <td>{new Date(task.createdAt).toLocaleString('zh-CN')}</td>
        <td>{task.owner?.name || task.owner?.username || task.ownerId.slice(0, 8)}<br /><small>{task.owner?.email || task.ownerId}</small></td>
        <td>{task.model}</td><td>{task.deletedAt ? '用户已隐藏' : task.status}</td><td>{task.prompt || '参考图生成'}</td>
        <td>{task.asset ? <a href={task.asset.original_url} target="_blank" rel="noreferrer">查看图片</a> : '-'}</td>
        <td>{task.asset && <button className="btn btn-secondary" type="button" onClick={() => void download(task.id)}>下载</button>}</td>
      </tr>)}</tbody></table></div>}
      {cursor && <button className="btn btn-secondary" type="button" onClick={() => void load(cursor)}>加载更多</button>}
    </section>
  </div>;
}
