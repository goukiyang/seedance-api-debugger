...
export function parseFeishuLocalRelay(value: unknown): FeishuLocalRelay | null {
  ...
  try {
    ...
    if (marker.searchParams.getAll('callback').length !== 1 || marker.searchParams.getAll('next').length !== 1) {
      return null;
    }
    if (Array.from(marker.searchParams.keys()).some((key) => key !== 'callback' && key !== 'next')) return null;

    const callbackValue = marker.searchParams.get('callback');
    const next = marker.searchParams.get('next');
    if (!callbackValue || !isSafeRelativePath(next)) return null;

    const callbackUrl = parseLoopbackCallback(callbackValue);
    return callbackUrl ? { callbackUrl, next } : null;
  } catch {
    return null;
  }
}
