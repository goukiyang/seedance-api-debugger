...
function referenceLabelForAsset(type: string, index: number) {
...
export async function renderPromptWithAssets(
  ...
  // 按素材类型建立 图片/视频/音频说明，图片仍兼容旧的 图N 写法。
  workspaceAssets.forEach((wa) => {
    typeCounts[wa.asset.type] = (typeCounts[wa.asset.type] || 0) + 1;
    const label = referenceLabelForAsset(wa.asset.type, typeCounts[wa.asset.type]);
    assetMapping[label] = wa.asset_id;
    assetMapping[`(${label})`] = wa.asset_id;
    if (wa.asset.type === 'image') {
      const imageLabel = `图片${typeCounts[wa.asset.type]}`;
      assetMapping[imageLabel] = wa.asset_id;
      assetMapping[`(${imageLabel})`] = wa.asset_id;
    }
  });
  ...
  const descTypeCounts: Record<string, number> = {};
  const refDescParts = workspaceAssets.map((wa) => {
    descTypeCounts[wa.asset.type] = (descTypeCounts[wa.asset.type] || 0) + 1;
    const label = referenceLabelForAsset(wa.asset.type, descTypeCounts[wa.asset.type]);
...
export async function validatePromptReferences(
  ...
  const imageCount = workspaceAssets.filter((item) => item.asset.type === 'image').length;
