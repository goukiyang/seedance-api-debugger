...
type PresetDraft = {
  ...
  bannerAssetId?: unknown; referenceIds?: unknown;
};
...
export async function listStudioPresets(userId: string, _isAdmin: boolean) {
  ...
  return rows.map(row => {
    ...
    return {
      ...
      model: row.model, quality: normalizeImageStudioQuality(row.model, row.quality), count: row.count,
...
export async function saveStudioPreset(userId: string, body: PresetDraft, isAdmin: boolean) {
  ...
  if (!name || name.length > 80 || prompt.length > 20000 || context.length > 20000 || groupName.length > 40 || !model || !Number.isInteger(count) || count < 1 || count > 8) throw new StudioModuleError('模板内容无效');
  ...
  return prisma.imageStudioPreset.create({ data: { id: randomUUID(), owner_id: userId, scope, name, group_name: groupName, prompt, context, model, quality, count, aspect_ratio: aspectRatio, banner_asset_id: bannerAssetId, reference_ids: JSON.stringify(ids) } });
}
...
export async function applyStudioPreset(userId: string, presetId: string, isAdmin: boolean) {
  ...
  return saveStudioModule(userId, { id: randomUUID(), revision: 0, name: preset.name, prompt: preset.prompt, context: preset.context, model: preset.model, quality: preset.quality, count: preset.count, aspectRatio: preset.aspect_ratio, groupName: preset.group_name, bannerAssetId: preset.banner_asset_id ? assetMap.get(preset.banner_asset_id) : null, referenceIds: ids.map(id => assetMap.get(id)).filter(Boolean) }, false, isAdmin);
}
