...
本节覆盖本回执中此前将无参数本地预览描述为“本地 Mock”的历史结论。当前行为以本节、提交 `91d1505`、`418a2ad`、`e6cddc5` 以及本轮最终审查修复为准：默认预览不生成假结果，测试 Mock 只能通过显式 `--mock-generation` 启用。
...
以完整历史区间 `9937708^..HEAD` 统计，本次设计、实现、测试和回执范围包含以下 14 个 tracked 文件：
...
- `ultimate-canvas-preview-server.mjs` 默认关闭假生成，对文字、图片、视频、预估、重试、状态、缩略图、审批创建和任务版本选择等路径返回 HTTP 503 与统一的 `REAL_BACKEND_REQUIRED` 消息；默认项目响应将 `_count.tasks` 归零，项目标签不再暴露预置任务数，卡片内嵌 project 也使用同一归一化。只有字面量 `--mock-generation` 开关启用测试 Mock。
- `ultimate-canvas-preview-no-generation-smoke.ts` 覆盖默认 bootstrap、能力禁用、稳定 503、无假资源/任务、隐藏任务隔离、审批拒绝、项目计数归零及卡片内嵌 project 不泄露。它还主动向子进程注入 `ULTIMATE_CANVAS_MOCK=1`、`MOCK_GENERATION=true` 和 `ENABLE_MOCK=true`，并断言后端仍为 `preview + mock:false`。
...
- 默认卡片 PATCH 带 `candidate_task_id` 时实际返回 400，预期 503。
- 默认 `POST /api/approvals` 实际返回 201 并创建 `approval-mock-*`，预期 503。
- 默认 bootstrap 中 `project-personal._count.tasks` 实际为 4，预期 0。
...
本轮最终审查修复的三个 smoke 和 `node --check` 均退出 0；其中默认 smoke 证明上述环境变量不能启用 Mock，显式 `--mock-generation` 的 API smoke 仍通过原有任务选择、审批 fixture 和生成生命周期。本轮未重跑部署、真实生成、全量 build 或 lint，不将历史验证扩大表述为本轮新证据。
...
- 本轮实现与验证回执已推送到 `teammate/ultimate-canvas-complete`；仍需由 SD2 部署流程发布后，生产地址才会显示新布局。
