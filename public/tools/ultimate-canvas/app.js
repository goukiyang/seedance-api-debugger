...
(function () {
    ...
    function applyGenerationResult(nodeEl, payload, result) {
        ...
        if (payload.kind === 'image' || payload.kind === 'video') {
            const title = payload.kind === 'video' ? '视频生成接口' : '图片生成接口';
                    ...
                    <div class="director-studio-main">
                        ...
                        <div class="director-studio-center">
                            ...
                            <div class="studio-bottom-toolbar">
                                <button class="active" title="移动视图/角色" aria-label="移动视图或角色" data-toolbar-tip="移动视图 / 拖动角色"><span>⌁</span><small>移动</small></button>
    ...
    function decorateGeneratedNode(nodeId, title, description, previewImage = '') {
        ...
        body.innerHTML = `
            <div class="generated-reference-card">
                ...
                ${previewImage
                    ? `<img class="generated-frame-preview" src="${previewImage}" alt="${title}">`
    ...
    document.addEventListener('click', (e) => {
        ...
        switch (action.dataset.directorAction) {
            ...
            case 'aspect':
                ...
                break;
            case 'ai-import':
                setDirectorStatus(document.querySelector(`[data-node-id="${sourceNodeId}"]`), 'AI识图导入入口已就绪');
                break;
            ...
            case 'add-crowd':
                ...
                break;
            case 'add-geometry':
                setDirectorStatus(document.querySelector(`[data-node-id="${sourceNodeId}"]`), '几何模型菜单已打开');
                closeStudioAddRoleMenu(nodeEl);
                break;
            case 'panorama':
            ...
            case 'storyboard':
                ...
                setDirectorStatus(nodeEl, '已生成参考图');
                break;
            case 'video':
                createDirectorOutput(sourceNodeId, 'video', '导演台首尾帧视频', `使用当前机位调度作为关键帧参考，接入视频生成节点。${directorSummary(nodeEl)}`);
                setDirectorStatus(nodeEl, '已接入视频节点');
                break;
            case 'grid': {
                const shots = [
