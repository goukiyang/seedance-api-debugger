...
contains(appSource, 'data-generation-setting-choice=', 'specification settings use the shared capability-backed choice contract');
...
contains(appSource, "generationChoiceGroup('watermark'", 'watermark setting exists');
...
contains(appSource, 'scheduleCanvasSave(`${node.type}_settings_change`)', 'image and video settings share one persisted settings path');
