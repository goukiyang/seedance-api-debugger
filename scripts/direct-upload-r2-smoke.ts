...
async function run() {
  ...
  assert.match(clientTicketSource, /fetch\('\/api\/assets\/upload-ticket'[\s\S]+catch \(error\)[\s\S]+uploadStageConnectionMessage\('上传票据创建', error\)[\s\S]+uploadWithRawFallbackOrThrow\(file, invalidJsonMessage, fallbackToRaw, message, onProgress(?:, uploadContext)?\)/, 'ticket connection errors must show upload-ticket stage context');
  assert.match(clientTicketSource, /readUploadJsonResponse<DirectUploadTicketResponse>\(ticketRes, '上传票据接口', invalidJsonMessage\)[\s\S]+catch \(error\)[\s\S]+uploadWithRawFallbackOrThrow\(file, invalidJsonMessage, fallbackToRaw, message, onProgress(?:, uploadContext)?\)/, 'ticket non-json errors must fallback safely or show a clear bounded error');
  ...
  assert.match(clientCompleteSource, /fetch\('\/api\/assets\/upload-complete'[\s\S]+catch \(error\)[\s\S]+uploadStageConnectionMessage\('上传完成登记', error\)[\s\S]+uploadWithRawFallbackOrThrow\(file, invalidJsonMessage, fallbackToRaw, message, onProgress(?:, uploadContext)?\)/, 'complete connection errors must show upload-complete stage context');
  assert.match(clientCompleteSource, /readUploadJsonResponse<UploadAssetResponse>\(completeRes, '上传完成登记接口', invalidJsonMessage\)[\s\S]+catch \(error\)[\s\S]+uploadWithRawFallbackOrThrow\(file, invalidJsonMessage, fallbackToRaw, message, onProgress(?:, uploadContext)?\)/, 'complete non-json errors must fallback safely or show a clear bounded error');
