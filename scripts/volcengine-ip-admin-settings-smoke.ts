...
const saved = buildVolcengineIpApiSettingsPatch(current, {
  ...
  api_key: 'secret-volc-key-1234567890',
});
...
assert.equal(saved.api_key, 'secret-volc-key-1234567890');
...
assert.equal(JSON.stringify(dto).includes('secret-volc-key'), false);
...
async function main() {
  ...
  const standaloneSource = fs.readFileSync(
    `${process.cwd()}/src/app/admin/integrations/volcengine-ip/page.tsx`,
    'utf8',
  );
