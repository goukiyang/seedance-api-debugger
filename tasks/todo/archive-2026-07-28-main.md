# Main todo archive 2026-07-28

- Source: `/Volumes/Data/Projects/video-api-debugger-v12-full-todo/tasks/todo.md`
- Lines: 7667
- Bytes: 546944
- Backup: `/Volumes/Data/Projects/video-api-debugger-v12-full-todo/tasks/todo/backups/todo-20260728-232548.md`
...
- [x] 在 `/assets` 页面新增“上传素材”条，支持图片和视频文件，使用统一 `uploadFileAsAsset` helper。
...
用户现在遇到的不是单张图片坏了，而是上传链路设计不适合线上：文件先进入 Next.js 服务，再由服务端转存对象存储，容易被表单解析、服务超时、Cloudflare 链路和后端内存拖慢。上一版 raw upload 已经止血，能避免 `request.formData()` 卡死后返回 HTML；本专项目标是把主链路升级成“浏览器直接传 R2，对象存储返回公网地址，后端只负责鉴权、校验、入库和审计”，让图片/视频/音频上传稳定、可显示真实进度，并保留旧链路作为回退。

做到完成的标准：`/generate` 上传历史图片、模板绑定图片、工作台参考图上传优先走 R2 直传；成功后能立刻出现在上传历史和当前参考图里；失败时能明确区分“文件不合规 / 存储上传失败 / 入库失败 / 需要重新登录”；线上真实登录态上传不再出现“服务返回页面内容”。
...
- [x] 独立只读审查：实现后创建独立只读审查 agent，审查 agent 不改文件、不提交、不补实现；只按本清单判断目标是否达标、证据是否充分、风险是否遗漏，并输出通过/不通过、证据、缺口和下一步。第一次审查不通过后已修复；复审通过。
- [x] 如果子 agent 暂不可用：由主线程按同一清单只读复查，不能改文件；该结果不是独立审查，可信度低于子 agent，不能把“独立只读审查”项标记为完成。本轮子 agent 可用，已完成独立只读复审。

### 4. 审查对齐检查

- [x] 审查必须围绕“浏览器直传对象存储，网站服务器不再搬大文件”这个目标，不要只检查中文错误提示。复审确认：当前代码已具备直传能力，但线上因 R2 CORS 未配置先安全关闭。
- [x] 审查必须确认用户操作习惯不变：仍然是点击上传或拖入/粘贴图片，系统自动走最优链路，不要求用户理解 R2、ticket、签名 URL。
- [x] 审查必须覆盖失败恢复：R2 不可用、登录失效、文件不合规、入库失败时，用户都能知道下一步怎么做。
- [x] 审查必须确认没有扩大范围：不顺手重做资产管理页、不改点数、不改视频生成 Provider、不改权限体系、不把所有旧上传入口一次性重构。
...
- 当前工作树：`/Volumes/Data/Projects/video-api-debugger-v12-full-todo`。
- 当前分支：`codex/seedream-5-pro-image-provider`。
- 建议任务分支：`codex/direct-r2-asset-upload`，避免和旧 `tasks/*.md` 脏改混在同一提交里。
- 提交分组：
  - 提交 1：R2 ticket / complete 接口和存储 helper。
  - 提交 2：前端 direct upload helper 与生成页、模板绑定入口接入。
  - 提交 3：错误提示、fallback、smoke 和真实验收修复。
- 发布前创建 rollback tag：`rollback/2026-07-20-before-direct-r2-upload`。

## 2026-07-15 WallVerse 剩余三组带声预演片

### 1. 目标复述

第一组「世界迁移」已经通过独立验收。本轮继续完成第二至第四组，每组由两条 12 秒视频组成，只在完整镜头之间切段，最终分别交付约 24 秒的带声预演片。三组必须共同证明 WallVerse 不是动态屏保：它会把家庭历史长成地貌、把家的状态变成世界法则、把当天的家庭素材写成只属于这个家的故事。

### 2. 具体任务

- [x] 第二组「记忆地貌」：生成 `迹城012`、`成境013`，合成带声版并加入卖点尾板。
- [x] 第三组「家是法则」：生成 `家律014`、`星眠015`，合成带声版并加入卖点尾板。
- [x] 第四组「今夜成真」：生成 `今夜016`、`鲸归017`，合成带声版并加入卖点尾板。
- [x] 每次付费生成前清空工作台参考绑定；每组第二条只允许使用本组第一条的真实尾帧。
- [x] 核对六条任务状态、扣点、退款、引用链和最终工作台活动参考数。

### 3. 独立只读验收

- [x] 已交给固定审核线程 `审核001 - sd2 固定只读审查`，结论通过、无阻塞问题。审核线程只读核对了六条源片、三条带声组片、四组合辑、接触表、完整解码、音轨、扣点和参考链路，记录已追加到 `tasks/audit-001-review.md`。

### 4. 审查对齐检查

- [x] 独立审查确认三组在无字幕时能看懂因果关系，并能分别复述“家庭物件变成世界地貌”“现实家居状态决定世界自然法则”“孩子当天的画变成今晚只属于这个家的故事”三个独占卖点。
- [x] 停止条件复核通过：未发现非本组参考、引用串组、付费失败未退款、镜头被任务边界截断、源片无法解码或核心卖点不可读。
