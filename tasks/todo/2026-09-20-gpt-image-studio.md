...
- 防复发：`ops/server/sd2/preflight.sh` 新增三条运行目录软链、持久目录、目录写权限检查；`ops/server/sd2/cutover-commands.md` 将发布前/切换后的两次 preflight 写成同一发布的强制门禁；`scripts/server-runtime-dirs-smoke.ts` 固化脚本和入口检查。以后 preflight 遇到普通目录、目标不在 `/data/video-api-debugger/var-lib` 或 `gouki` 不可写时直接失败，不进入发布验收。
