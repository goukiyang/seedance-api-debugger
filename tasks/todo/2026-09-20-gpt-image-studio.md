...
发布证据（2026-09-24）：代码提交 `5c9414831c3cc91b4d81ac66876d2d5142446a9e`，版本 `0.14.0`，线上 `BUILD_ID=frg4u3kQXvvVr4OjFOqlD`；`/api/release`、`/api/config`、`/api/health`、`/login` 均正常，匿名 `/api/image-studio/presets` 返回 401、匿名 `/image-studio` 跳转登录，原始素材 URL 与未知受控资产 URL 返回 404。上传目录实际解析到 `/data/video-api-debugger/var-lib/uploads/assets`，目录为 `gouki:gouki` 且服务进程以 `gouki:gouki` 运行，写权限检查通过，已覆盖截图中的 `mkdir .../public/uploads/assets` 权限错误。`sd2-gray.service`、`sd2-image-studio.service` 在健康周期后均 active 且 `NRestarts=0`。生产备份：`/srv/video-api-debugger/backups/release-0.14.0-5c94148/dev.db`，quick check 通过；回退 tag：`rollback/2026-09-24-before-image-template-sharing-0.14.0`。Chrome 登录态因 `CHROME_DEVTOOLS_HTTP_404` 未取得，未读取或输出任何 cookie/token。
