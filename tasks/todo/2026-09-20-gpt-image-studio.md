...
| 编号 | 任务 | 完成标准 | 状态 |
|---|---|---|---|
| T1 | Banner 满宽适配 | 模板 Banner 撑满内容区宽度，按原图比例自适应，不被固定最大高度截断 | 已实现，候选构建通过，待发布验收 |
| T2 | 模板库可见性 | 登录用户可以查看并应用管理员及其他创建者的模板；应用后复制成自己的模块配置 | 已实现，候选构建通过，待发布验收 |
| T3 | 保存权限与入口 | 当前配置保存按钮改为手动可感知状态，未改动灰显，改动后亮起；另存为合并为一个入口 | 已实现，候选构建通过，待发布验收 |
| T4 | 生成图片访问边界 | 生成结果通过登录和用户归属校验的接口读取，匿名不能直接查看生成结果 | 已实现，API 与旧 `/uploads` 访问守门均已补齐，待发布验收 |
| T5 | 构建发布 | 版本、Git、服务器候选构建、服务、公网接口和关键页面资源一致 | 进行中 |
