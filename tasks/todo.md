...
- [ ] 审查必须围绕“浏览器直传对象存储，网站服务器不再搬大文件”这个目标，不要只检查中文错误提示。
- [ ] 审查必须确认用户操作习惯不变：仍然是点击上传或拖入/粘贴图片，系统自动走最优链路，不要求用户理解 R2、ticket、签名 URL。
- [ ] 审查必须覆盖失败恢复：R2 不可用、登录失效、文件不合规、入库失败时，用户都能知道下一步怎么做。
- [ ] 审查必须确认没有扩大范围：不顺手重做资产管理页、不改点数、不改视频生成 Provider、不改权限体系、不把所有旧上传入口一次性重构。
