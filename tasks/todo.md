...
- [ ] 为 `Project` 或独立预算表补齐预算字段：`budget_credits`、`budget_used_credits`、`budget_frozen_credits`、`budget_currency`、`budget_status`。
- [ ] 抽象预算账户：个人项目扣个人积分，公共项目扣项目预算，系统项目按现有规则单独处理。
- [ ] 生成任务创建时先做预算预占，写入冻结快照和幂等键。
- [ ] 任务成功后实扣项目预算，并写入成本/积分流水。
- [ ] 技术失败、Provider 创建失败、任务取消时释放预占或返还。
- [ ] 公共项目预算不足时阻止生成，并给出追加预算入口。
- [ ] 项目页展示预算总额、已用、冻结、剩余、预算状态和 70% / 90% / 100% 风险提示。
- [ ] 后台成本页增加公共预算视角：按项目、视频卡、负责人、时间范围查看消耗。
...
- [ ] 公共项目预算不足时生成请求被拒绝。
- [ ] 技术失败会释放冻结预算。
- [ ] 重复提交同一幂等键不会重复冻结或重复扣费。

## 任务包 C：审批中心与 1080p 真审批

目标：把当前轻量确认升级为可追溯审批系统。

- [ ] 新增 `ApprovalRecord` 数据结构。
- [ ] 审批类型包含：公共项目审批、追加预算审批、1080p 审批、比例变更审批、视频卡重开审批。
- [ ] 审批记录关联 `project_id`、`video_card_id`、`task_id`、`requester_user_id`、`approver_user_id`、`status`、`reason`、`approved_at`。
- [ ] 新增审批中心页面，支持待我审批、我发起的、已通过、已拒绝、已失效。
- [ ] 非个人项目 1080p 生成不再只靠前端复选框，后端必须校验有效审批。
- [ ] 追加预算、比例变更、视频卡重开接入同一审批中心。
- [ ] 审批通过、拒绝、过期时写入操作日志。

验收：

- [ ] 没有有效 1080p 审批时不能生成 1080p。
- [ ] 审批通过后只能在审批约束范围内生成，例如限定项目、视频卡、分辨率和有效期。
- [ ] 审批拒绝后生成页展示明确原因。
- [ ] 管理员能追溯审批来源、理由和处理人。
...
- [ ] 修改 `src/app/generate/page.tsx` 最近任务渲染：时间和提示词摘要放在同一行信息结构中，提示词不另起一整块抢占首屏。
- [ ] 修改 `src/app/globals.css` 对应最近任务样式：桌面端一行可扫描，移动端允许合理换行但不能让时间和提示词断成两个独立区块。
- [ ] 抽出或补齐相对时间 helper，覆盖 `刚刚`、`N 分钟前`、`N 小时前`、`N 天前`、`N 周前`、`N 个月前`。
- [ ] 保留绝对时间可追溯性，建议通过 `title` 或详情页保留，不把审计信息完全丢掉。

验收：

- [ ] `/generate` 最近任务在桌面端能一眼看到“相对时间 + 提示词摘要”同一行。
- [ ] 创建时间分别为 2 天、10 天、45 天前的任务显示为相对时间，不直接显示日期。
- [ ] 移动端无横向溢出，提示词不会遮挡状态、缩略图或操作入口。
...
- [ ] 复核 `src/components/GenerationComposer.tsx` 的 `hasAttemptedSubmit` 与空 prompt 校验链路，区分“引导输入”和“提交失败”。
- [ ] 复核 `src/components/ComposerStatusLine.tsx` 与 `src/app/globals.css` 的 `hint | progress | ok | error` 样式，空 prompt 优先保持 hint/中性提示。
- [ ] 只有服务端拒绝、素材映射错误、余额不足、Provider 创建失败等真实阻断问题才使用 error 红色。

验收：

- [ ] 初次进入 `/generate` 时空提示词为蓝色或中性引导。
- [ ] 用户点提交但仍未输入 prompt 时，不出现强错误态红色，除非还有其他真实错误。
- [ ] 真实提交失败仍有明确红色错误反馈。
...
- [ ] 复核 `src/components/ImageSetToolbar.tsx` 及生成页引用入口，确认“新建图集”是否和“保存当前素材为图集”语义重复。
- [ ] 将生成页主入口调整为面向当前任务的动作，例如“保存当前素材为图集”；已有图集的重命名/删除继续放在图集列表或详情页。
- [ ] 如果生成页需要管理已有图集，只提供明确跳转或轻量管理入口，不在生成工具栏里重复完整图集后台。

验收：

- [ ] 用户在 `/generate` 能明确区分“把当前素材保存成图集”和“管理已有图集”。
- [ ] 图集重命名、删除入口仍可达。
- [ ] 不删除原有创建图集能力，只调整入口语义和层级。
...
- [ ] 复核 `src/app/projects/[id]/video-cards/[cardId]/page.tsx` 的生成记录列表，生成者展示统一改为 `UserIdentityBadge`。
- [ ] 复核 `src/app/projects/[id]/page.tsx` 的项目内任务/历史记录，涉及生成者时统一使用头像 + 显示名。
- [ ] 如 API 缺少 `avatar_url` 或 `account_type`，只补当前列表需要的最小字段，不扩大敏感数据返回。

验收：

- [ ] 任务详情、产出留存、视频卡生成记录、项目内任务记录的生成者展示口径一致。
- [ ] 无头像用户有稳定 fallback 首字母或默认头像，不造成布局跳动。
- [ ] 技术邮箱和合成飞书账号仍按 `src/lib/users/display.ts` 过滤显示。
