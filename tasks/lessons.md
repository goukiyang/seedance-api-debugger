...
- 问题/背景：资产管理页 `/assets` 是客户端页面，最初读取 URL 查询参数来设置 `type=video` 初始筛选。
- 诱因/根因：`tsc` 和 `lint` 都能通过，但 Next 生产构建会额外执行 App Router 预渲染检查；客户端搜索参数、动态路由或临时 distDir 缓存问题只有生产构建能暴露。
